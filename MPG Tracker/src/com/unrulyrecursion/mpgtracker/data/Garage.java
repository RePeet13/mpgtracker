...
/**
 * This class will act as a singleton for interacting with the database, as well
 * as holding the database's reference.
 * 
 * @author Taylor
 * 
 */
...
//		close();
	...
	public void addCar(Car car) { // TODO add return value based on success
		// TODO check against current carlist to see if its already in it, later
		// regen carArray
		...
		Log.i("Garage", "Car Mileage: " + car.getTotalMileage());
		cv.put(DBContract.COLUMN_NAME_TOTAL_MILEAGE, car.getTotalMileage());
	...
	public void updateCar(Car car) { // TODO add return value based on success
		...
		// TODO Could add toast that says updated or something
		open();
	...
	public List<Car> getAllCars() { // TODO this should set carList, not return stuff
	...
	public Car cursorToCar(Cursor cursor) {
		...
		int totalMileage = cursor.getInt(3);
		Log.i("Garage", "Car Mileage: " + totalMileage);

		Car car = new Car(carId, carName, color, make, model, year,
				totalMileage);
	...
	public Car[] getCarArray() {
		// TODO dirty checking based on whether stuff has been written to the
		// database or not
